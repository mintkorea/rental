...
# 2. CSS 설정 (이미지 d888bc 수준의 깔끔한 디자인으로 복구)
st.markdown("""
<style>
...
# 7. PDF 생성 (나눔고딕 폰트 적용 및 레이아웃 최적화)
def create_pdf(df, title):
    ...
    pdf.set_fill_color(68, 68, 68) # 테이블 헤더 어두운 회색
    pdf.set_text_color(255, 255, 255) # 흰색 글자
    cols = ["날짜", "건물명", "장소", "시간", "행사명", "상태"]
    widths = [20, 35, 40, 40, 110, 20]
    ...
    for _, row in df.iterrows():
        ...
        # 행사명이 길 경우 잘림 방지를 위해 간략화
        event_nm = str(row['행사명'])[:48] + ".." if len(str(row['행사명'])) > 50 else str(row['행사명'])
        pdf.cell(widths[4], 9, event_nm, border=1, align='L')
