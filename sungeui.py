...
# 3. 데이터 수집 로직 (백지화 방지를 위한 예외 처리 강화)
...
def get_data(target_date):
    ...
    try:
        ...
        for item in raw:
            ...
            # 요일 필터링 (데이터가 아예 없거나 None이면 무조건 노출, 아니면 현재 요일 체크)
            allow_day_raw = str(item.get('allowDay', '')).lower()
            if allow_day_raw and allow_day_raw != 'none':
...
st.markdown(f"""
    <div style="border-bottom:2px solid #333; padding-bottom:10px; margin-bottom:20px;">
        ...
        <p style='font-size:1.1rem;'>
            {s_date.strftime("%Y. %m. %d")}(<span style='color:{color}'>{wd_names[wd_idx]}</span>) &nbsp; | &nbsp; 근무조 : {shift_info}
        </p>
    </div>
    """, unsafe_allow_html=True)

# 데이터 유무와 관계없이 건물별 섹션 출력
f_df = df[df['건물명'].isin(sel_bu)].copy() if not df.empty else pd.DataFrame()

with st.sidebar:
    st.write("---")
    # 빈 데이터라도 엑셀 생성 가능하도록 처리
...
for b in sel_bu:
    st.markdown(f"#### 📍 {b}")
    b_df = f_df[f_df['건물명'] == b] if not f_df.empty else pd.DataFrame()
