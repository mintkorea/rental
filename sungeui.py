...
def get_data(s_date, e_date):
    ...
    try:
        ...
        for item in raw:
            ...
            curr = start_dt
            while curr <= end_dt:
                # 선택한 시작일~종료일 범위 내에 있는지 확인
                if s_date <= curr <= e_date:
                    # 요일 제한이 없거나, 현재 날짜의 요일이 허용된 요일에 포함될 때
                    if not allowed_days or (curr.isoweekday()) in allowed_days:
                        rows.append({
...
df = get_data(s_date, s_date) # 시작/종료일을 동일하게 조회일로 설정
...
if not df.empty:
    ...
    for b in sel_bu:
        ...
        st.markdown(f"#### 📍 {b}")
        if not b_df.empty:
            # 중앙 정렬을 위해 column_config 활용
            st.dataframe(
                b_df[['장소', '시간', '행사명', '부서', '부스', '인원', '상태']],
                use_container_width=True,
                hide_index=True,
                column_config={
                    "시간": st.column_config.TextColumn(help="중앙 정렬", width="small"),
