...
def create_final_pdf(df, title_text, selected_buildings):
    ...
    # 선택된 건물들만 필터링
    filtered_df = df[df['건물명'].isin(selected_buildings)].copy()
    
    if not filtered_df.empty:
        # 날짜별로 먼저 나누고, 그 안에서 건물별로 출력
        for date_val in sorted(filtered_df['full_date'].unique()):
            date_df = filtered_df[filtered_df['full_date'] == date_val]
