...
# 건물 출력 순서 (GitHub 최신 변경사항 반영)
BUILDING_ORDER = ["성의회관", "의생명산업연구원", "옴니버스 파크", "옴니버스파크 의과대학", "옴니버스파크 간호대학", "대학본관", "서울성모별관"]

# 2. 데이터 가져오기 및 요일 필터링 (핵심 수정 부분)
...
def get_data(s_date, e_date):
    ...
    try:
        ...
        for item in raw:
            ...
            # allowDay 분석 (예: "1,2,3" -> [1, 2, 3])
            allow_day_raw = str(item.get('allowDay', ''))
            # 숫자가 아닌 값이 섞여있을 경우를 대비한 안전한 변환
            allowed_weekdays = [int(d.strip()) for d in allow_day_raw.split(',') if d.strip().isdigit()]
            ...
            while curr <= e_dt:
                ...
                if s_date <= curr <= e_date:
                    # 요일 체크: 파이썬 weekday(월=0...일=6)를 API 기준(월=1...일=7)으로 변환
                    curr_weekday_api = curr.weekday() + 1
                    
                    # allowed_weekdays가 비어있으면 매일 허용, 값이 있으면 포함 여부 확인
                    if not allowed_weekdays or curr_weekday_api in allowed_weekdays:
                        rows.append({
...
if not data.empty:
    try:
        ...
        st.sidebar.error(f"PDF 생성 에러: {e}")
