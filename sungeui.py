...
def get_data(s_date, e_date):
    ...
    try:
        ...
        for item in raw:
            ...
            # 데이터 파싱 로직 실행...
            rows.append({'날짜': item['startDt'], '건물명': item['buNm'], '장소': item['placeNm'], 
                         '시간': f"{item['startTime']}~{item['endTime']}", '행사명': item['eventNm'],
                         '인원': item.get('peopleCount', '-'), '부서': item.get('mgDeptNm', '-'), '상태': '확정'})
...
if df is not None and not df.empty:
    ...
    st.write("조회된 데이터가 없습니다. 날짜나 건물 필터를 확인해 주세요.")
