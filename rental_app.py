...
def update_google_sheet(df):
    ...
    try:
        ...
        for _, r in df.sort_values(['full_date', '시간']).iterrows():
            ...
            values.append([r['full_date'], "월화수목금토일"[t_dt.weekday()], get_shift(t_dt), "기간" if r['is_period'] else "당일", r['건물명'], r['장소'], r['시간'], r['행사명'], r['부서'], r['인원'], r['상태']])
...
def create_excel(df, selected_bu):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        c_fmt = workbook.add_format({'border': 1, 'align': 'center', 'text_wrap': True})
        worksheet.merge_range('A1:F1', "성의교정 대관 현황", t_fmt)
        row = 2
        for d_str in sorted(df['full_date'].unique()):
            worksheet.merge_range(row, 0, row, 5, f"📅 {d_str}", h_fmt); row += 1
            b_df = df[(df['full_date'] == d_str) & (df['건물명'].isin(selected_bu))]
...
with st.expander("🔍 조회 및 데이터 관리", expanded=True):
    ...
    with c3:
        ...
        if not df.empty:
            st.download_button("📊 Excel", create_excel(df, sel_bu), f"대관_{s_date}.xlsx", use_container_width=True)
            if st.button("🚀 시트 동기화", use_container_width=True, type="primary"):
                if update_google_sheet(df): st.success("✅ 동기화 완료!")

# --- 데이터 표시부 (오류 수정 및 가로 표 최적화) ---
if not df.empty:
    curr = s_date
    while curr <= e_date:
        d_str = curr.strftime('%Y-%m-%d')
        # KeyError 방지: '날짜' 대신 'full_date' 사용
            ...
            for bu in sel_bu:
                ...
                if view_mode == "가로 표":
                    ...
                    <table class="custom-table">
                        <thead><tr><th style="width:15%">장소</th><th style="width:12%">시간</th><th style="width:38%">행사명</th><th style="width:15%">부서</th><th style="width:10%">인원</th><th style="width:10%">상태</th></tr></thead>
                        <tbody>
                    """
                    ...
                    for _, r in b_df.sort_values('시간').iterrows():
                        st.markdown(f'<div class="mobile-card" style="border-left:5px solid {"#2196F3" if r["is_period"] else "#2ecc71"};"><div class="row-1"><span class="loc-text">📍 {r["장소"]}</span><span class="time-text">🕒 {r["시간"]}</span><span class="status-badge">{r["상태"]}</span></div><div class="row-2"><b>{r["행사명"]}</b> / {r["부서"]} ({r["인원"]}명)</div>{"<div style=\'font-size:11px; color:#2196F3; margin-top:5px;\'>🗓️ "+r["period_range"]+" ("+r["allowed_days"]+")</div>" if r["is_period"] else ""}</div>', unsafe_allow_html=True)
        curr += timedelta(days=1)
else:
