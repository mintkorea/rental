...
# 2. CSS 설정 (다크모드 대응 및 요일 색상)
st.markdown("""
<style>
    /* 다크모드 지원을 위해 고정 배경색 제거 및 투명도 조절 */
    ...
    th { background-color: rgba(128, 128, 128, 0.1); border: 1px solid #dee2e6; padding: 6px; font-size: 11px; font-weight: bold; }
    td { border: 1px solid rgba(128, 128, 128, 0.1); padding: 8px 6px; font-size: 12px; text-align: center; }
</style>
""", unsafe_allow_html=True)

# 3. 데이터 로드 (기존 성공 소스 로직 그대로 유지)
                ...
                if s_date <= curr <= e_date:
                    ...
                    if not allowed_weekdays or curr_weekday in allowed_weekdays:
                        rows.append({
                            ...
                            'weekday_num': curr.weekday(), # 5: 토, 6: 일
...
def create_split_pdf(df, selected_buildings):
    ...
    for date_val in sorted(df['full_date'].unique()):
        ...
        for bu in selected_buildings:
            ...
            pdf.set_font("Nanum", size=12); pdf.set_text_color(46, 80, 119)
            pdf.cell(0, 10, f"■ {bu}", ln=True, align='L'); pdf.set_text_color(0, 0, 0)
            cols = [("장소", 40), ("시간", 35), ("행사명", 115), ("인원", 12), ("부서", 50), ("상태", 15)]
            pdf.set_fill_color(240, 240, 240); pdf.set_font("Nanum", size=10)
            for txt, width in cols: pdf.cell(width, 9, txt, border=1, align='C', fill=True)
            pdf.ln(); pdf.set_font("Nanum", size=9)
...
if not all_df.empty:
    for date in sorted(all_df['full_date'].unique()):
        ...
        w_num = day_df.iloc[0]['weekday_num']
        # 요일 색상 클래스 적용
        ...
        for bu in selected_bu:
            ...
            if not bu_df.empty:
                ...
                # 기존 성공 소스의 HTML 테이블 구조 유지 (인덱스 없음)
