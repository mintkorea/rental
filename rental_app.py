...
# --- 1. 엑셀 생성 (수정 반영: 헤더 높이 조절 및 수직 정렬 강화) ---
def create_excel(df, selected_bu):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.set_row(0, 40) # 제목 줄 높이
...
with st.expander("🔍 설정 및 다운로드", expanded=True):
    ...
    with c3:
        ...
        if not df.empty:
            ...
            # 보완: 파일명 조회 기간 반영 (가변 처리)
            file_dt = f"{s_date}_{e_date}" if s_date != e_date else f"{s_date}"
            sc1.download_button("📊 Excel", create_excel(df, sel_bu), f"성의교정_대관현황({file_dt}).xlsx", use_container_width=True)
            sc2.download_button("📄 CSV", create_csv(df), f"성의교정_대관현황({file_dt}).csv", use_container_width=True)
