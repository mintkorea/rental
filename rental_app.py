            ...
            while curr <= e_dt:
                if s_date <= curr <= e_date:
                    ...
                    rows.append({
                        ...
                        '건물명': safe_t(item.get('buNm', '')).strip(),
...
# 4. 독립 뷰어용 HTML 생성 (줌 가능하게 메타 태그 삽입)
html_code = f"""
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        .d-box {{ font-size: 18px; font-weight: bold; background: #f0f2f6; padding: 10px; border-left: 6px solid #2e5077; margin-top: 25px; }}
        .b-tag {{ font-size: 15px; font-weight: bold; margin: 15px 0 5px 0; color: #333; }}
        ...
        .w-p {{ width: 120px; }} .w-t {{ width: 85px; }} .w-e {{ width: auto; }}
...
if not raw_df.empty:
    ...
    for d in sorted(filtered_df['full_date'].unique()):
        ...
        html_code += f'<div class="d-box">📅 {d} ({day_df.iloc[0]["요일"]})</div>'
        for bu in BUILDING_ORDER:
            if bu in selected_bu:
                bu_df = day_df[day_df['건물명'] == bu]
                if not bu_df.empty:
                    html_code += f'<div class="b-tag">🏢 {bu}</div>'
                    html_code += """<div class="t-wrap"><table><thead><tr>
    ...
    html_code += "<p style='text-align:center;'>조회된 데이터가 없습니다.</p>"

html_code += "</body></html>"

# 5. 출력 (높이를 넉넉히 잡아 스크롤 대신 줌 유도)
