...
<style>
    ...
    .date-sun { color: #FF0000 !important; } /* 일/공휴일 적색 */
    ...
    /* 표 레이아웃 강제 고정 및 모바일 감응 해제 */
    .table-container { width: 100%; overflow-x: auto; }
    table { width: 100%; border-collapse: collapse; table-layout: fixed !important; min-width: 850px; }
    th, td { border: 1px solid rgba(128, 128, 128, 0.2); padding: 4px 2px; text-align: center; vertical-align: middle; }
    
    /* 열 너비 비율 고정 (시간 필드 최소화) */
    ...
    td { font-size: 12px; word-break: break-all; line-height: 1.2; }
...
# 3. 데이터 로드 (성공했던 기존 소스 로직 100% 복구 - 절대 수정 금지)
...
def create_split_pdf(df, selected_buildings):
    ...
    for date_val in sorted(df['full_date'].unique()):
        ...
        pdf.cell(0, 15, f"대관 현황 ({date_val} {date_df.iloc[0]['요일']}요일)", ln=True, align='C')
        for bu in selected_buildings:
            ...
            for _, row in bu_df.iterrows():
                pdf.cell(40, 9, str(row['장소'])[:20], border=1)
                pdf.cell(35, 9, str(row['시간']), border=1, align='C')
                pdf.cell(115, 9, " " + str(row['행사명'])[:50], border=1)
                pdf.cell(12, 9, str(row['인원']), border=1, align='C')
                pdf.cell(50, 9, str(row['부서'])[:20], border=1)
