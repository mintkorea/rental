...
st.markdown("""
    <style>
    /* 상단 여백: 모바일 브라우저 간섭 방지를 위해 충분히 확보 */
    ...
    /* 내역 없음 메시지 스타일 (건물 소제목 아래용) */
    .empty-building-msg { color: #888; padding: 15px 10px; font-size: 14px; font-style: italic; }
    
    /* 전체 결과 없음 스타일 */
    .no-data-overall { background-color: #f8f9fa; color: #666; padding: 30px; border-radius: 10px; text-align: center; border: 1px dashed #ccc; margin-top: 20px; }
...
if not df_res.empty:
    for d_str in sorted(df_res['full_date'].unique()):
        ...
        for bu in sel_bu:
            # 건물 헤더 출력 (내역 유무와 상관없이 무조건 출력)
            b_df = df_res[(df_res['full_date'] == d_str) & (df_res['건물명'].str.replace(" ", "") == bu.replace(" ", ""))]
            count = len(b_df)
            st.markdown(f'<div class="building-header"><div style="font-size:17px; font-weight:bold; color:#1e3a5f;">🏢 {bu}</div><div class="count-text">총 {count}건</div></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="no-data-overall">🔍 조회된 대관 내역이 전혀 없습니다.</div>', unsafe_allow_html=True)
