...
# 1. 사용자님 원본 디자인 및 설정 (100% 복구)
st.set_page_config(page_title="성의교정 대관 현황", page_icon="🏫", layout="wide")
KST = pytz.timezone('Asia/Seoul')
now_today = datetime.now(KST).date()import streamlit as st
import requests
import pandas as pd
from datetime import datetime, date, timedelta
import pytz
...
st.markdown("""
    ...
    .card-event { font-size: 15px; font-weight: 700; color: #333; margin-top: 5px; }
    .card-info { font-size: 13px; color: #777; margin-top: 3px; }
    </style>
""", unsafe_allow_html=True)

# 엑셀 생성 함수 (가이드라인 반영: 행 높이 35, 지정된 열 너비)
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for d_str in sorted(df['full_date'].unique()):
            ...
            for bu in selected_bu:
                b_df = df[(df['full_date'] == d_str) & (df['건물명'] == bu)]
...
with st.expander("🔍 설정 및 엑셀 다운로드", expanded=True):
    ...
    df = get_data(s_d, e_d)
    if not df.empty:
        st.download_button("📥 엑셀 파일 저장", data=create_excel(df, bu_s), file_name=f"대관현황_{s_d}.xlsx")
