...
st.markdown("""
    ...
    .fixed-table th, .fixed-table td {
        ...
        overflow: hidden;
        line-height: 1.2;
        height: 40px; /* 셀 높이 균일화 */
    }
    .fixed-table th { background-color: #f8f9fa; font-weight: bold; color: #333; }

    /* 지정하신 컬럼 너비 비율 적용 (장소:1, 시간:0.8, 행사:2, 부서:1, 상태:0.5) */
    .col-place { width: 22%; } /* 장소명 최대 길이 기준 */
    .col-time { width: 15%; }
    .col-event { width: 40%; } /* 장소명의 약 2배 */
    .col-dept { width: 22%; }  /* 장소명과 동일 */
    .col-status { width: 10%; }

    /* 자동 개행 및 폰트 축소 로직 (CSS 단에서 처리) */
    .cell-content {
        display: -webkit-box;
    ...
    /* 데이터가 너무 길 경우에 대비한 미세 폰트 조정 */
    .long-text { font-size: 11px !important; }

    .empty-building-msg { color: #999; padding: 10px 5px; font-size: 13.5px; }
        ...
        for bu in sel_bu:
            ...
            if count > 0:
                ...
                for _, r in b_df.iterrows():
                    # 텍스트 길이에 따른 폰트 클래스 부여 (약 15자 이상일 때 축소)
                    p_class = "long-text" if len(r['장소']) > 14 else ""
                    e_class = "long-text" if len(r['행사명']) > 25 else ""
                    d_class = "long-text" if len(r['부서']) > 14 else ""
                    
                    table_html += f"""
                        <tr>
                            <td class="{p_class}"><div class="cell-content">{r['장소']}</div></td>
                            <td>{r['시간']}</td>
                            <td class="{e_class}"><div class="cell-content">{r['행사명']}</div></td>
                            <td class="{d_class}"><div class="cell-content">{r['부서']}</div></td>
