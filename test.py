...
st.markdown("""
    <style>
    .block-container { padding-top: 1.5rem !important; max-width: 1000px !important; margin: 0 auto !important; }
    
    /* 메인 타이틀 디자인 */
    .main-header { font-size: 26px; font-weight: bold; color: #1e3a5f; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; border-bottom: 3px solid #1e3a5f; padding-bottom: 10px; }
    
    /* 날짜 소타이틀 (확대 반영) */
    .date-shift-bar {
        background-color: #444; color: white; padding: 12px; border-radius: 8px;
        text-align: center; margin: 25px 0 15px 0; font-weight: bold; 
...
if not df.empty:
    ...
    st.info("조회된 대관 내역이 없습니다. 사이드바에서 날짜를 확인해 주세요.")
