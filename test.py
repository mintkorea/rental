                ...
                if bu in selected_buildings:
                    ...
                    if not b_df.empty:
                        ...
                        for _, r in b_df.iterrows():
                            worksheet.set_row(curr_row, 35)
                            worksheet.write(curr_row, 0, r['장소'], l_f)
                            worksheet.write(curr_row, 1, r['시간'], c_f)
                            worksheet.write(curr_row, 2, r['행사명'], l_f)
                            worksheet.write(curr_row, 3, r['부서'], l_f)
                            worksheet.write(curr_row, 4, r['인원'], c_f)
                            worksheet.write(curr_row, 5, r['부스'], c_f)
...
# [복구] 엑셀 다운로드 버튼을 본문 상단으로 이동
if not df.empty:
    excel_data = create_formatted_excel(df, sel_bu)
    st.download_button("📥 전체 내역 엑셀 다운로드", data=excel_data, file_name=f"대관현황_{s_date}.xlsx", use_container_width=False)

    for d_str in sorted(df['full_date'].unique()):
        d_obj = datetime.strptime(d_str, '%Y-%m-%d').date()
        st.markdown(f'<div style="background-color:#444; color:white; padding:10px; border-radius:5px; margin-top:25px; font-weight:bold;">'
                    f'🗓️ {d_str} | 근무조: {get_shift(d_obj)}</div>', unsafe_allow_html=True)
        
        for bu in sel_bu:
            b_df = df[(df['full_date'] == d_str) & (df['건물명'].str.replace(" ","") == bu.replace(" ",""))]
            if not b_df.empty:
                # [복구] 건물명 옆에 '총 X건' 표시 복구
