...
if not df_result.empty:
    # [핵심 수정] column_config를 통한 헤더 및 데이터 중앙 정렬 강제 적용
    col_config = {
        "장소": st.column_config.TextColumn("장소", width="medium"),
        "시간": st.column_config.TextColumn("시간", width="small", alignment="center"), # 정렬 추가
        "행사명": st.column_config.TextColumn("행사명", width="large"),
        "부서": st.column_config.TextColumn("부서", width="medium"),
        "상태": st.column_config.TextColumn("상태", width="small", alignment="center")  # 정렬 추가
    }
        ...
        for bu in sel_bu:
            ...
            if not b_df.empty:
                if view_mode == "가로 모드 (표)":
                    # 헤더 정렬은 Streamlit 기본 사양에 따라 column_config를 따름
                    st.dataframe(
