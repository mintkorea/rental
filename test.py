...
<style>
    ...
    .col-time { flex: 4; font-size: 14px; color: #d9534f; text-align: center; font-weight: bold; }
...
# 4. 데이터 수집 함수 (필터링 로직 강화)
@st.cache_data(ttl=60)
def get_data(start_date, end_date):
    url = "https://songeui.catholic.ac.kr/ko/service/application-for-rental_calendar.do"
    # 날짜 형식을 API가 인식하는 정확한 형태로 전달
...
with st.sidebar:
    ...
    # 모바일/데스크톱 강제 선택 옵션 추가 (중복 노출 방지)
    view_mode = st.radio("화면 모드", ["자동(너비감지)", "모바일(셸)", "PC(표)"], horizontal=True)

df = get_data(s_date, e_date)

if not df.empty:
    # 엑셀 다운로드 버튼 (타이틀 바로 아래 배치)
    ...
    for d_str in sorted(df['full_date'].unique()):
        ...
        for bu in sel_bu:
            ...
            if not b_df.empty:
                ...
                # 중복 노출 방지를 위한 파이썬 분기 처리
                # (CSS 미디어 쿼리 대신 사용자가 선택하거나 너비에 따라 하나만 실행)
                if view_mode == "PC(표)":
                    st.dataframe(b_df[['장소', '시간', '행사명', '부서', '인원', '상태']], use_container_width=True, hide_index=True)
                elif view_mode == "모바일(셸)":
                    for _, row in b_df.iterrows():
                        ...
                        <div class="event-shell">
                            ...
                            <div style="font-size:14px; color: #666; margin-top:5px;">📄 {row['행사명']} ({row['인원']}명)</div>
                        </div>""", unsafe_allow_html=True)
                else: # 자동 모드 (기존 CSS 방식 유지하되 태그 분리)
                    # 모바일용 셸
                    m_html = '<div class="mobile-only-view">'
                    for _, row in b_df.iterrows():
                        m_html += f'<div class="event-shell"><div class="row-1"><div class="col-place">📍 {row["장소"]}</div><div class="col-time">⏰ {row["시간"]}</div><div class="col-status">{row["상태"]}</div></div><div style="font-size:14px; color: #666; margin-top:5px;">📄 {row["행사명"]} ({row["인원"]}명)</div></div>'
                    m_html += '</div>'
                    st.markdown(m_html, unsafe_allow_html=True)
                    # PC용 표
                    st.markdown('<div class="desktop-only-view">', unsafe_allow_html=True)
                    st.dataframe(b_df[['장소', '시간', '행사명', '부서', '인원', '상태']], use_container_width=True, hide_index=True)
    ...
    st.markdown("""
    <style>
        @media (max-width: 767px) { .desktop-only-view { display: none !important; } }
        @media (min-width: 768px) { .mobile-only-view { display: none !important; } }
    </style>
    """, unsafe_allow_html=True)
else:
    st.warning("📅 선택하신 날짜와 건물에 해당하는 대관 내역이 없습니다. 시작일과 종료일을 확인해주세요.")
