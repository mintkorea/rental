...
# 2. CSS 스타일 (모바일 첫 행 통합 및 PC 표 가독성)
st.markdown("""
<style>
    ...
    .col-place { flex: 5.5; font-size: 15px; font-weight: 700; color: #1e3a5f; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
            ...
            if not b_df.empty:
                ...
                if "모바일" in view_mode:
                    for _, row in b_df.iterrows():
                        ...
                        st.markdown(f"""<div class="event-shell"><div class="row-main"><div class="col-place">📍 {row['장소']}</div><div class="col-time">⏰ {row['시간']}</div><div class="col-status" style="color:{st_color};">{row['상태']}</div></div><div class="row-sub">📄 {row['행사명']} ({row['부서']}, {row['인원']}명)</div></div>""", unsafe_allow_html=True)
                else:
