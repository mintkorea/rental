...
with st.sidebar:
    ...
    # [핵심] 사용자가 직접 가로(표) / 세로(카드)를 선택하게 함
    view_mode = st.radio("📱 보기 모드 설정", ["모바일 (카드형)", "PC (표 형식)"], index=0)
...
while curr_day <= e_date:
    ...
    for bu in sel_bu:
        ...
        if not b_df.empty:
            # 선택된 모드에 따라 단 하나의 블록만 출력
            if view_mode == "PC (표 형식)":
                # 가로 모드 최적화 (표)
